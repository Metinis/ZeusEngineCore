...
SampledImage::SampledImage(const ImageCreateInfo& createInfo,
                           CommandBlock commandBlock, Bitmap const& bitmap)
{
	...
	Image image(createInfo, usage, mipLevels, vk::Format::eR8G8B8A8Srgb, extent);
	...
	const Buffer stagingBuffer(bufferCreateInfo, BufferMemoryType::Host, bitmap.bytes.size_bytes());
