...
DeviceBuffer::DeviceBuffer(BufferCreateInfo const& createInfo,
                           CommandBlock commandBlock, ByteSpans const& byteSpans)
{
	...
	Buffer stagingBuffer(stagingCreateInfo, BufferMemoryType::Host, totalSize);

	Buffer deviceBuffer(createInfo, BufferMemoryType::Device, totalSize);
